...
class PerformanceAndStressSuite(unittest.TestCase):
    ...
    def test_sanitize_error_message_masks_all_secret_keys(self):
        ...
        dirty_messages = [
            ("Error en llamada con AIzaSyAbCdEfGhIjKlMnOpQrStUvWxYz1234567 y fallo", "[CLAVE_ENMASCARADA]"),
            ("HTTP 401: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.xyz", "Bearer [TOKEN_ENMASCARADO]"),
    ...
    def test_ai_service_masks_secrets_in_exception_and_url(self):
        ...
        fake_err = Exception("Google GenAI 403 Forbidden: url https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash:generateContent?key=AIzaSyC_TEST_SECRET_KEY_12345678")
        with patch("ai_service.genai.Client") as mock_client:
            mock_client.return_value.models.generate_content.side_effect = fake_err
            res = analyze_with_gemini(["falla modulador"], self.search_engine, api_key="AIzaSyC_TEST_SECRET_KEY_12345678")
            self.assertFalse(res.get("ok"))
            msg = res.get("message", "")
            self.assertNotIn("AIzaSyC_TEST_SECRET_KEY", msg)
