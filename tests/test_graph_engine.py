...
class GraphEngineSuite(unittest.TestCase):
    ...
    def test_graph_file_integrity_and_memory_footprint(self):
        ...
        self.assertLess(size_mb, MAX_GRAPH_PAYLOAD_MB, f"El grafo pesa {size_mb:.2f} MB; debe mantenerse < {MAX_GRAPH_PAYLOAD_MB} MB para móviles")

        self.assertGreaterEqual(len(self.engine.entities), MIN_GRAPH_ENTITIES, f"Debe tener al menos {MIN_GRAPH_ENTITIES} entidades técnicas")
        self.assertGreaterEqual(len(self.engine.adjacency), MIN_GRAPH_ADJACENCY, f"Debe tener al menos {MIN_GRAPH_ADJACENCY} listas de adyacencia")
