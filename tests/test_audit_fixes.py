...
class AuditFixesVerificationSuite(unittest.TestCase):
    ...
    def test_circuit_matcher_high_performance_and_no_truncation(self):
        """match_subsystem_for_trace procesa 300 componentes en < 30ms sin descartar componentes reales."""
        ...
        self.assertLess(elapsed_ms, 30.0, f"Matcher tardó {elapsed_ms:.2f}ms (límite 30ms)")
    ...
    def test_strict_zero_visible_ai_in_html_and_templates(self):
        ...
        if template_file.is_file():
            with open(template_file, "r", encoding="utf-8") as f:
                html_code = f.read()
            # Remover bloques de script/style internos para evaluar texto visible al usuario
            visible_text = re.sub(r"<(script|style)[^>]*>.*?</\1>", " ", html_code, flags=re.DOTALL | re.IGNORECASE)
            visible_text = re.sub(r"<[^>]+>", " ", visible_text)
            self.assertNotRegex(
                visible_text,
                r"\b(?:IA|AI|Inteligencia\s+Artificial)\b",
                "Violación de regla estricta: mención visible de IA/AI en index.html"
            )
