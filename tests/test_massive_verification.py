...
from search_engine import SearchEngine, normalize, tokens
...
class MassiveSOLVITestSuite(unittest.TestCase):
    ...
    def test_03_knowledge_graph_parity_and_size(self):
        ...
        self.assertLess(size_mb, 2.0, f"El grafo pesa {size_mb:.2f} MB, debe ser menor a 2.0 MB para móviles")

        # Entidades y adyacencias
        self.assertGreaterEqual(len(self.graph_engine.entities), 1500)
        self.assertGreaterEqual(len(self.graph_engine.adjacency), 1000)
    ...
    def test_04_massive_search_queries_and_false_positive_rejection(self):
        ...
        self.assertLess(avg_ms, 50.0, f"La latencia promedio ({avg_ms:.2f} ms) debe ser < 50 ms")
